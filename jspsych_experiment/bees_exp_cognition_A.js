...
// Model's predictions for pred_samples (Task 2)
...
var instructions = {
	...
	our model's prediction (marked red in title).</p>
...
var just_protos = {
	...
	while <i>B. lucorum</i> only has <b>two</b>.
	Throughout the whole experiment you will be shown these representative samples next to the test samples to help you
	decide which species the test sample belongs to. The Venn diagram will <b>not</b> be shown again.</p><br>
...
for (let i = 0; i < pred_samples.length; i++) {
  pred_stimuli.push({stimulus: pred_samples[i], correct_response: classes[i], pred_class: preds[i]});
};
...
var debrief_block_no_model_1 = {
    ...
    stimulus: function() {
      ...
      return `<p>Num. trials: ${trials.count()}, out of which ${correct_trials.count()} were correct.</p>
        ...
        <p><strong>Task 2 (6 images): Assign a species while also knowing our model's prediction (marked red in title).</strong></p>`;
    },
    choices: ['Continue']
};
